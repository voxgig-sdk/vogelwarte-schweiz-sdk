...
// VogelwarteSchweiz SDK feature test
//
...
class FtCtrl extends VogelwarteSchweizControl
{
...
class FtClient
{
    ...
    private VogelwarteSchweizUtility $utility;

    public function __construct(VogelwarteSchweizUtility $utility, array $options)
    {
    ...
    public function get_utility(): VogelwarteSchweizUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (VogelwarteSchweizContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public VogelwarteSchweizContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (VogelwarteSchweizContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = VogelwarteSchweizConfig::make_feature($name);
    ...
    private static function buildUrl(VogelwarteSchweizSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof VogelwarteSchweizError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new VogelwarteSchweizResponse($fetched);
        }
    ...
    private function fail(VogelwarteSchweizContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (VogelwarteSchweizContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
