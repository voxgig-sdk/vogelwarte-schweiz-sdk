...
// VogelwarteSchweiz SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = VogelwarteSchweizOmni::makeStructRunner(
    ...
    {
        return VogelwarteSchweizOmni::objmodel(VogelwarteSchweizOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return VogelwarteSchweizOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return VogelwarteSchweizOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(VogelwarteSchweizOmni::structfix($now, Struct::undef()))
                        !== json_encode(VogelwarteSchweizOmni::structfix($val, Struct::undef()))) {
