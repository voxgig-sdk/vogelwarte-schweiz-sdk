...
func TestBirdEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		birdRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.bird")))
		...
		birdRef01DataDt0LoadResult := core.ToMapAny(entityData(birdRef01DataDt0Loaded))
...
func birdBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID"])
