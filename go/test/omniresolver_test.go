...
import (
	...
	omni "github.com/voxgig-sdk/vogelwarte-schweiz-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.VogelwarteSchweizError); is {
				out := map[string]any{
					"name":    "VogelwarteSchweizError",
...
func omniCtx(arg any, client *sdk.VogelwarteSchweizSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.VogelwarteSchweizSDK); is {
			client = live
