...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("VOGELWARTESCHWEIZ_TEST_LIVE") == "TRUE" ||
		os.Getenv("VOGELWARTESCHWEIZ_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("VOGELWARTESCHWEIZ_TEST_EXPLAIN"); explain != "" {
