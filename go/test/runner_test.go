...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("VOGELWARTE_SCHWEIZ_TEST_LIVE") == "TRUE" ||
		os.Getenv("VOGELWARTE_SCHWEIZ_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("VOGELWARTE_SCHWEIZ_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.VogelwarteSchweizSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.VogelwarteSchweizSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.VogelwarteSchweizError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.VogelwarteSchweizSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.VogelwarteSchweizError{Msg: msg, Code: code}
}
