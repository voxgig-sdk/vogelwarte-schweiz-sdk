...
require("vogelwarte-schweiz_types")
...
local VogelwarteSchweizSDK = {}
VogelwarteSchweizSDK.__index = VogelwarteSchweizSDK
...
VogelwarteSchweizSDK._make_feature = _make_feature


function VogelwarteSchweizSDK.new(options)
  local self = setmetatable({}, VogelwarteSchweizSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function VogelwarteSchweizSDK:options_map()
...
function VogelwarteSchweizSDK:get_utility()
  return Utility.copy(self._utility)
end


function VogelwarteSchweizSDK:get_root_ctx()
  return self._rootctx
end


function VogelwarteSchweizSDK:prepare(fetchargs)
...
function VogelwarteSchweizSDK:direct(fetchargs)
...
function VogelwarteSchweizSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function VogelwarteSchweizSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "VogelwarteSchweizSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function VogelwarteSchweizSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function VogelwarteSchweizSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "VogelwarteSchweizSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function VogelwarteSchweizSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = VogelwarteSchweizSDK.new(sdkopts)
  sdk.mode = "test"
