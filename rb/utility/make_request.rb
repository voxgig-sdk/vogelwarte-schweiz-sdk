...
module VogelwarteSchweizUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = VogelwarteSchweizResponse.new({})
    ...
    if fetch_err
      ...
      response = VogelwarteSchweizResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = VogelwarteSchweizResponse.new(fetched)
    else
