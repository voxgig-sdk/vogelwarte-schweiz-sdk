...
require_relative 'VogelwarteSchweiz_types'
...
class VogelwarteSchweizSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = VogelwarteSchweizHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = VogelwarteSchweizHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, VogelwarteSchweizFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    VogelwarteSchweizUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = VogelwarteSchweizHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = VogelwarteSchweizHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = VogelwarteSchweizHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => VogelwarteSchweizError.new(
        "#{op}_allow",
        "VogelwarteSchweizSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue VogelwarteSchweizError => err
    ...
    if fetched.is_a?(Hash)
      status = VogelwarteSchweizHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = VogelwarteSchweizError.new(
        "graphql_error", "VogelwarteSchweizSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = VogelwarteSchweizSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
