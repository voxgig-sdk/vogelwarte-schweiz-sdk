...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = VogelwarteSchweizOmni.make_runner(TEST_JSON_FILE, VogelwarteSchweizSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    VogelwarteSchweizError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : VogelwarteSchweizOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = VogelwarteSchweizOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
