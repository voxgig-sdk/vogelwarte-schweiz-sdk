...
class BirdEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = VogelwarteSchweizSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    bird_ref01_data_dt0_load_result = Helpers.to_map(bird_ref01_data_dt0_loaded.respond_to?(:data_get) ? bird_ref01_data_dt0_loaded.data_get : bird_ref01_data_dt0_loaded)
...
def bird_basic_setup(extra)
  ...
  entid_env_raw = ENV["VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID"]
