...
//    only - a typed VogelwarteSchweizContext there would make every ctx
...
final class VogelwarteSchweizOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new VogelwarteSchweizContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new VogelwarteSchweizSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new VogelwarteSchweizResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new VogelwarteSchweizError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new VogelwarteSchweizResponse($clean['response']);
    ...
    public static function err_from_map($m): ?VogelwarteSchweizError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new VogelwarteSchweizError(is_string($code) ? $code : '', $msg);
    }
