...
// VogelwarteSchweiz SDK pipeline test
//
...
{
    private static function utility(): VogelwarteSchweizUtility
    {
        return new VogelwarteSchweizUtility();
    }
    ...
    private static function ctx(array $over = []): VogelwarteSchweizContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new VogelwarteSchweizContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new VogelwarteSchweizOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof VogelwarteSchweizError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new VogelwarteSchweizOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = VogelwarteSchweizMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new VogelwarteSchweizOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = VogelwarteSchweizMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new VogelwarteSchweizOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = VogelwarteSchweizMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VogelwarteSchweizSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = VogelwarteSchweizMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new VogelwarteSchweizResponse([]),
            'result' => new VogelwarteSchweizResult([]),
        ]);
        [, $err] = VogelwarteSchweizMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new VogelwarteSchweizSpec([]),
            'response' => null,
            'result' => new VogelwarteSchweizResult([]),
        ]);
        [, $err] = VogelwarteSchweizMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new VogelwarteSchweizSpec(['step' => 's']),
            'response' => new VogelwarteSchweizResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new VogelwarteSchweizResult([]),
        ]);
        [, $err] = VogelwarteSchweizMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new VogelwarteSchweizSpec(['step' => 's']),
            'response' => new VogelwarteSchweizResponse(self::resp(200, ['v' => 1])),
            'result' => new VogelwarteSchweizResult([]),
        ]);
        [, $err] = VogelwarteSchweizMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new VogelwarteSchweizSpec(['step' => 's']),
            'response' => new VogelwarteSchweizResponse(self::resp(200, ['v' => 2])),
            'result' => new VogelwarteSchweizResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        VogelwarteSchweizMakeResponse::call($ctx);
    ...
    {
        $preset = new VogelwarteSchweizResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new VogelwarteSchweizSpec([]),
            'response' => new VogelwarteSchweizResponse([]),
            'result' => new VogelwarteSchweizResult([]),
        ...
        [$got, $err] = VogelwarteSchweizMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new VogelwarteSchweizResult([])]);
        [, $err] = VogelwarteSchweizMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new VogelwarteSchweizSpec([]), 'result' => null]);
        [, $err] = VogelwarteSchweizMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VogelwarteSchweizSpec(['step' => 's']),
            'result' => new VogelwarteSchweizResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new VogelwarteSchweizOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VogelwarteSchweizMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new VogelwarteSchweizSpec(['step' => 's']),
            'result' => new VogelwarteSchweizResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new VogelwarteSchweizOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = VogelwarteSchweizMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new VogelwarteSchweizResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new VogelwarteSchweizSpec([]),
            'result' => new VogelwarteSchweizResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = VogelwarteSchweizMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = VogelwarteSchweizMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new VogelwarteSchweizError('boom', 'boom');
        $utility->fetcher = function (VogelwarteSchweizContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VogelwarteSchweizSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = VogelwarteSchweizMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (VogelwarteSchweizContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = VogelwarteSchweizMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(VogelwarteSchweizResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        VogelwarteSchweizMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (VogelwarteSchweizContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new VogelwarteSchweizSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = VogelwarteSchweizMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new VogelwarteSchweizResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new VogelwarteSchweizSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = VogelwarteSchweizMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = VogelwarteSchweizMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = VogelwarteSchweizMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new VogelwarteSchweizResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, VogelwarteSchweizDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new VogelwarteSchweizResult(['ok' => false])]);
        $this->expectException(VogelwarteSchweizError::class);
        VogelwarteSchweizDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new VogelwarteSchweizResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', VogelwarteSchweizMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        VogelwarteSchweizMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new VogelwarteSchweizBaseFeature();
        $b = new VogelwarteSchweizBaseFeature();
        VogelwarteSchweizFeatureAdd::call($ctx, $a);
        VogelwarteSchweizFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): VogelwarteSchweizBaseFeature
    {
        $f = new VogelwarteSchweizBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        VogelwarteSchweizFeatureAdd::call($ctx, self::named_feature('a'));
        VogelwarteSchweizFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        VogelwarteSchweizFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        VogelwarteSchweizFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        VogelwarteSchweizFeatureAdd::call($ctx, $replace);
        ...
        VogelwarteSchweizFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): VogelwarteSchweizContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new VogelwarteSchweizSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = VogelwarteSchweizPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        VogelwarteSchweizPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new VogelwarteSchweizResponse(['headers' => null]),
            'result' => new VogelwarteSchweizResult([]),
        ]);
        VogelwarteSchweizResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        VogelwarteSchweizResultBody::call($ctx);
