...
class BirdEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $cfg = VogelwarteSchweizConfig::shared_config();
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $bird_ref01_data_dt0_load_result = Helpers::to_map(is_object($bird_ref01_data_dt0_loaded) && method_exists($bird_ref01_data_dt0_loaded, 'data_get') ? $bird_ref01_data_dt0_loaded->data_get() : $bird_ref01_data_dt0_loaded);
...
function bird_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["VOGELWARTE_SCHWEIZ_TEST_BIRD_ENTID"]);
    if ($idmap_resolved === null) {
        $idmap_resolved = Helpers::to_map($idmap);
    }

    if ($env["VOGELWARTE_SCHWEIZ_TEST_LIVE"] === "TRUE") {
    ...
    return [
        ...
        "env" => $env,
        "explain" => $env["VOGELWARTE_SCHWEIZ_TEST_EXPLAIN"] === "TRUE",
        "live" => $live,
