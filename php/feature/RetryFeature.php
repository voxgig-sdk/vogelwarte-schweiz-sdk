...
// VogelwarteSchweiz SDK retry feature
...
class VogelwarteSchweizRetryFeature extends VogelwarteSchweizBaseFeature
{
    ...
    public function init(VogelwarteSchweizContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (VogelwarteSchweizContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(VogelwarteSchweizContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(VogelwarteSchweizContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
