...
// VogelwarteSchweiz SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class VogelwarteSchweizTestFeature extends VogelwarteSchweizBaseFeature
{
    ...
    public function init(VogelwarteSchweizContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (VogelwarteSchweizContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = VogelwarteSchweizParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (VogelwarteSchweizContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(VogelwarteSchweizContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = VogelwarteSchweizParam::call($ctx, $k);
