# VogelwarteSchweiz SDK test runner
...
class VogelwarteSchweizTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            VogelwarteSchweizTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = VogelwarteSchweizTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = VogelwarteSchweizTestRunner.getenv("VOGELWARTE_SCHWEIZ_TEST_LIVE")
        override = VogelwarteSchweizTestRunner.getenv("VOGELWARTE_SCHWEIZ_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = VogelwarteSchweizTestRunner.getenv(key)
        ...
        explain = VogelwarteSchweizTestRunner.getenv("VOGELWARTE_SCHWEIZ_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["VOGELWARTE_SCHWEIZ_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if VogelwarteSchweizTestRunner._test_control is not None:
            return VogelwarteSchweizTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                VogelwarteSchweizTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            VogelwarteSchweizTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = VogelwarteSchweizTestRunner.load_test_control()
...
def load_env_local():
    VogelwarteSchweizTestRunner.load_env_local()


def env_override(m):
    return VogelwarteSchweizTestRunner.env_override(m)


def entity_data(v):
    return VogelwarteSchweizTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return VogelwarteSchweizTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return VogelwarteSchweizTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return VogelwarteSchweizTestRunner.load_test_control()


def live_client_options():
    return VogelwarteSchweizTestRunner.live_client_options()


def live_delay_ms():
    return VogelwarteSchweizTestRunner.live_delay_ms()
