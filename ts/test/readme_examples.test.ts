...
//     (\`new VogelwarteSchweizSDK(...)\` / \`VogelwarteSchweizSDK.test(...)\` become
//     \`VogelwarteSchweizSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'VogelwarteSchweizSDK'
...
const TEST_SEED = {"entity":{"bird":{"test01":{"id":"test01"}},"species":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new VogelwarteSchweizSDK(...)\` and any
// \`VogelwarteSchweizSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+VogelwarteSchweizSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/VogelwarteSchweizSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
