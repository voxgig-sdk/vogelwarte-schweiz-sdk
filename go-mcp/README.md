# vogelwarte-schweiz-mcp

[MCP](https://modelcontextprotocol.io) server exposing the VogelwarteSchweiz SDK as
two agent tools — `vogelwarte-schweiz_list` and `vogelwarte-schweiz_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vogelwarte-schweiz-mcp)
make build

# 2. Provide credentials via the environment
export VOGELWARTE_SCHWEIZ_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user vogelwarte-schweiz \
  -- /absolute/path/to/vogelwarte-schweiz-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./vogelwarte-schweiz-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// vogelwarte-schweiz_list: first page of records
{ "entity": "bird" }
{ "entity": "bird", "query": { } }

// vogelwarte-schweiz_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vogelwarte-schweiz-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user vogelwarte-schweiz \
     -- "$PWD"/dist/*/vogelwarte-schweiz-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `vogelwarte-schweiz_list` and `vogelwarte-schweiz_load` tools now appear
   in new sessions. Ask the agent to *"list bird using vogelwarte-schweiz"*
   and it calls `vogelwarte-schweiz_list` with `{"entity":"bird"}`.
...
```sh
export VOGELWARTE_SCHWEIZ_APIKEY=sk_live_xxx            # API key
export VOGELWARTE_SCHWEIZ_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `vogelwarte-schweiz_list` tool
...
### Call the `vogelwarte-schweiz_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `vogelwarte-schweiz_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `vogelwarte-schweiz_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `VOGELWARTE_SCHWEIZ_APIKEY` | API key sent with every request. |
| `VOGELWARTE_SCHWEIZ_BASE` | Optional override of the API base URL. |
...
```sh
./vogelwarte-schweiz-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"vogelwarte-schweiz_load","arguments":{"entity":"bird","query":{"id":1}}}}'
```
