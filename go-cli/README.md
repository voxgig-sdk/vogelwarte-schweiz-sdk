# vogelwarte-schweiz-cli

boru-driven command-line client **and** interactive REPL for the VogelwarteSchweiz
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/vogelwarte-schweiz-cli)
make build

# 2. See usage (words, entities, env vars)
./vogelwarte-schweiz-cli --help

# 3. Provide credentials once, via the environment
export VOGELWARTE_SCHWEIZ_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./vogelwarte-schweiz-cli list bird
./vogelwarte-schweiz-cli load 1 bird            # {id:1} shorthand
./vogelwarte-schweiz-cli load '{id:1}' bird       # explicit match map
./vogelwarte-schweiz-cli list species

# 5. Override the API base URL for a single call
VOGELWARTE_SCHWEIZ_BASE=https://api.example.com ./vogelwarte-schweiz-cli list bird

# 6. No arguments -> interactive REPL
./vogelwarte-schweiz-cli
vogelwarte-schweiz> list bird
vogelwarte-schweiz> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/vogelwarte-schweiz-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/vogelwarte-schweiz-cli list bird
   ```
...
```sh
./vogelwarte-schweiz-cli load 1 bird          # scalar shorthand for {id:1}
./vogelwarte-schweiz-cli load '{id:1}' bird     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `vogelwarte-schweiz>`). Each line is
evaluated as its own boru expression:

```text
$ ./vogelwarte-schweiz-cli
vogelwarte-schweiz> list bird
vogelwarte-schweiz> /help
vogelwarte-schweiz> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `bird`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/vogelwarte-schweiz-cli`. |
...
(`./vogelwarte-schweiz-cli <expr>`) and interactively (the REPL), and expressions compose the
